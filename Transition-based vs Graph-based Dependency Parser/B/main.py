...
from model_bert import BertBISTParser
...
def main():
    ...
    parser.add_argument('--pretrained_emb',
                        help='path to a pretrained word embeddings file', default=None)
    parser.add_argument('--activation_function', type=str, default='tanh',
                        help='name of the activation function to use in the MLPs (default: tanh)')
    parser.add_argument('--encoder', type=str, default='lstm',
                        help='type of the encoder to user. One of {"lstm", "bert"} (default: lstm)')
    ...
    if args.encoder == "lstm":
        ...
        parser = BISTParser(w_emb_dim=args.w_emb_dim,
                            ...
                            w_i_counter={w2i[w]: count for w, count in words_count.items() if w in w2i},
                            w2i=w2i,
                            p2i=p2i,
                            alpha=args.alpha,
                            device=args.device,
        ...
        # bert based parser initialization
        parser = BertBISTParser(pos_emb_dim=args.pos_emb_dim,
                            mlp_hid_dim=args.mlp_hid_dim,
                            n_arc_relations=len(r2i),
